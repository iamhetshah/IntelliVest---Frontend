...
const API_KEY = "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"; // Replace with environment variable in production
...
router.post('/general-advice', validateRequestBody ,userMiddleware, async (req, res , next) => {
  try {
...
router.post('/analyze-investment', validateRequestBody, userMiddleware, async (req, res, next) => {
  try {
    ...
    ${investmentType === 'stock' ? `- Quantity: ${stockQuantity}
    - Action: ${stockAction}
    - Current Price: ₹${currentPrice}` : ''}
    ${investmentType === 'ppf' ? `- Frequency: ${ppfFrequency}
    - Tenure: ${ppfTenure} years` : ''}
    ${investmentType === 'fd' ? `- Tenure: ${fdTenure} months
    - Interest Payout: ${fdInterestPayout}` : ''}
    ${investmentType === 'mutual_fund' ? `- Category: ${mfCategory}
    - Investment Type: ${mfInvestType}
    - Risk Profile: ${riskProfile}` : ''}
    ${investmentType === 'gold' ? `- Type: ${goldType}
